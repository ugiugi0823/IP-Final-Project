...
# CUDA_VISIBLE_DEVICES=4 python train.py --model_name DistgSSR  --angRes 5 --scale_factor 2 --batch_size 8
...
def train(train_loader, device, net, criterion, optimizer):
    ...
    for idx_iter, (data, label, data_info) in tqdm(enumerate(train_loader), total=len(train_loader), ncols=70):
        ...
        data = data.to(device)      # 저해상도 데이터
        label = label.to(device)    # 고해상도 데이터
